...
export class TemplateEngine {
    static formatWithMetaInfo(template: string, metaInfo: { subscriptionUrl: string; username: string }): string {
        return this.replaceLazy(template, {
            USERNAME: metaInfo.username,
            SUBSCRIPTION_LINK: metaInfo.subscriptionUrl,
            HAPP_CRYPT3_LINK: () => createHappCryptoLink(metaInfo.subscriptionUrl, 'v3', true) || 'unknown',
            HAPP_CRYPT4_LINK: () => createHappCryptoLink(metaInfo.subscriptionUrl, 'v4', true) || 'unknown',
        })
    }

    static replace(template: string, values: TemplateValues): string {
        let hasReplacement = false
        const result = template.replace(/\{\{(\w+)\}\}/g, (match, key: TSubscriptionPageTemplateKey) => {
