...
import { IconAlertCircle, IconArrowsUpDown, IconCalendar, IconCheck, IconUserScan, IconX } from '@tabler/icons-react'
...
export const SubscriptionInfoExpandedWidget = ({ isMobile }: IProps) => {
    ...
    const getStatusAndIcon = (): {
        ...
        if ((user.userStatus === 'ACTIVE' && user.daysLeft === 0) || (user.daysLeft >= 0 && user.daysLeft <= 3)) {
            return {
                color: 'orange',
        ...
        <Card p={{ base: 'sm', xs: 'md', sm: 'lg', md: 'xl' }} radius="lg">
            <Stack gap={isMobile ? 'sm' : 'md'}>
                <Group gap="sm" justify="space-between">
                    <Group gap={isMobile ? 'xs' : 'sm'} style={{ minWidth: 0, flex: 1 }} wrap="nowrap">
                        <ThemeIcon
                        ...
                        <Stack gap={2} style={{ minWidth: 0, flex: 1 }}>
                            ...
                            <Text c={user.daysLeft === 0 ? 'red' : 'dimmed'} fw={600} size={isMobile ? 'xs' : 'sm'}>
                ...
                <SimpleGrid cols={{ base: 2, xs: 2, sm: 2 }} spacing="xs" verticalSpacing="xs">
                    ...
                    <InfoBlock
                        ...
                        value={user.userStatus === 'ACTIVE' ? t(baseTranslations.active) : t(baseTranslations.inactive)}
                    />
